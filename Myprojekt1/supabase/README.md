...
Для текущего изменения применить в SQL Editor по порядку:

1. `migrations/20260831_registration_moderation.sql`;
2. `migrations/20260831_002_repair_product_image_rls.sql`;
3. `migrations/20260901_001_seed_ukraine_city_markets.sql`.

Перед применением сохранить резервную копию и проверить существующие строки
`shops`. Вторая миграция исправляет RLS для сохранения товаров и загрузки
фотографий, не разрешая пользователю писать в папку другого аккаунта.
Третья миграция безопасно добавляет Київ, Вінницю, Львів, Івано-Франківськ и
стартовые рынки этих городов; повторный запуск не создаёт дубликаты.
